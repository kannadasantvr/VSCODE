  ...
  it('Firstname', () => {
    ...
    cy.get('[class="um-field-checkbox-option"]').then(($checkbox) => {
      ...
      const transformToCSV = (data) => {
        const headers = Object.keys(data[0]).join(',');  // Header row (keys)
        const rows = data.map(item => Object.values(item).join(',')); // Data rows (values)
      ...
      // Write the CSV file
      cy.writeFile('cypress/results/AppVT.csv', csvData);
    })
  })
})
